...
if uploaded_file:
    ...
    # Plot word count distribution with user-selected words on x-axis
    st.subheader("Word Count Distribution with User-Selected Words")
    selected_words = st.multiselect("Select words:", word_count['Word'].tolist())
    selected_word_count = word_count[word_count['Word'].isin(selected_words)]
    plt.figure(figsize=(10, 6))
    sns.barplot(data=selected_word_count, x='Word', y='Count')
    plt.xlabel('Words')
    plt.ylabel('Count')
    plt.title('Word Count Distribution with User-Selected Words')
    plt.xticks(rotation=90)
    st.pyplot(plt)

        
    # Add your links here
    linkedin_link = "https://github.com/ZayabAkhtar"
    github_link = "https://www.linkedin.com/in/zayabakhtar/"

    # Display the links
    st.sidebar.subheader("Connect with me:")
    st.sidebar.markdown(f"[LinkedIn]({linkedin_link})  [GitHub]({github_link})")

    # Display the app creator
    st.sidebar.subheader("App created by:")
    st.sidebar.write("Zayab Akhtar")
